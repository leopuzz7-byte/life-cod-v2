...
async function generateSpreadCard(deckProfile, sphereLabel, question, position, index, cardDesc, prevDescs) {
  ...
  const combination = isLenormand && prevDescs && prevDescs.length
    ...
    : isDecameron
      ? "В трактовке разделяй физическое влечение, эмоциональную близость, намерение и реальное действие. Не считай желание доказательством любви, верности или готовности к отношениям."
      : "";
...
async function generateSpreadFinal(deckProfile, sphereLabel, question, descs, positions) {
  ...
  const lenormandTask = isLenormand
    ...
    : isDecameron
      ? "Покажи единую динамику между притяжением, эмоциональной потребностью и реальным поведением. Отдельно обозначь, где есть взаимность, а где желание может не совпадать с намерением или готовностью действовать. Не описывай интимные сцены и не объявляй измену фактом."
