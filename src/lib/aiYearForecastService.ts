import { YearForecast, calculatePersonalMatrix } from './calculations';
...
export interface AIYearReading {
  ...
  attention: string;          // на что обратить особое внимание
  avoid: string;              // чего лучше избегать
  matrixInteraction: string;  // взаимодействие аркана года с личной матрицей
  keyDates: string;           // ключевые даты года
  months: Record<string, AIYearMonthDetail>; // "1".."12"
}

export interface AIYearMonthDetail {
  theme: string;         // общая тема месяца + главная энергия
  moneyWork: string;     // работа и деньги
  relationships: string; // отношения
  health: string;        // здоровье и состояние
  chance: string;        // главный шанс месяца
  risk: string;          // главный риск месяца
  tip: string;           // практический совет
}

const CACHE_PREFIX = 'ai_year_v3_';
...
function buildCorePrompt(forecast: YearForecast, name: string): string {
  ...
  const pm = calculatePersonalMatrix(birthDate.day, birthDate.month, birthDate.year);
  const pmLine = pm.positions.map((v, i) => `поз.${i + 1} ${arcanaLabel(v)}`).join(', ');
  const karmic = [8, 10, 11, 20, 21].includes(arcana);
  return `Ты нумеролог системы Аркан-Коды. Пишешь персональный годовой прогноз. Язык: русский.
${name ? `КЛИЕНТ: ${name}\n` : ''}${yearContext(forecast)}

ЛИЧНАЯ МАТРИЦА КЛИЕНТА (12 позиций): ${pmLine}
Ключевые позиции: поз.1 суть, поз.4 мудрость и накопление, поз.6 карма и опора, поз.7 цель жизни.

Сначала раскрой сам аркан как архетип, затем покажи, как он будет проживаться именно этим человеком в этом году.
${karmic ? 'ВАЖНО: этот аркан кармический. Раскрой значительно глубже обычного: кармические уроки, судьбоносные события, точки выбора, последствия решений, влияние на дальнейшие годы жизни, возможности масштабного роста и риски упущенных шансов. Объём текста больше стандартного.\n' : ''}
${STYLE_RULES}
...
Верни СТРОГО JSON без markdown:
{
  ...
  "attention": "на что обратить особое внимание в этом году: главные точки внимания и сигналы (4-5 предложений)",
  "avoid": "чего лучше избегать в этом году: конкретные действия и решения (4-5 предложений)",
  "matrixInteraction": "взаимодействие аркана года ${arcanaLabel(arcana)} с личной матрицей клиента: что усиливается, где возможно напряжение, на какие позиции влияет сильнее всего (4-5 предложений)"
}`;
}
...
Верни СТРОГО JSON без markdown:
{
  ...
  "keyDates": "ключевые даты года: назови самые значимые месяцы и периоды, когда возможны важные события и повороты, с опорой на арканы месяцев (4-5 предложений)",
  "wish": "тёплое личное пожелание на следующий год (3-4 предложения)"
}`;
}

// Промт по месяцам (по сферам, как в файле). Возвращает JSON, ключи - номера месяцев.
function buildMonthsPrompt(forecast: YearForecast, name: string, startM: number, endM: number): string {
  const { birthDate, arcana } = forecast;
  const lines: string[] = [];
  const shape: string[] = [];
  for (let m = startM; m <= endM; m++) {
    const p1 = normalizeToArcana(birthDate.month + m);
    const p3 = normalizeToArcana(p1 + arcana);
    lines.push(`${MONTH_NAMES[m]} (${m}): треугольник ${arcanaLabel(p1)} + ${arcanaLabel(arcana)} = итог ${arcanaLabel(p3)}`);
    shape.push(`  "${m}": {"theme":"общая тема месяца и главная энергия итогового аркана (2-3 предложения)","moneyWork":"работа и деньги (2 предложения)","relationships":"отношения (2 предложения)","health":"здоровье и состояние (1-2 предложения)","chance":"главный шанс месяца (1-2 предложения)","risk":"главный риск месяца (1-2 предложения)","tip":"главный практический совет (1 предложение)"}`);
  }
  return `Ты нумеролог системы Аркан-Коды. Пишешь прогноз по месяцам ${forecast.targetYear} года. Язык: русский.
${name ? `КЛИЕНТ: ${name}\n` : ''}Годовой аркан: ${arcanaLabel(arcana)}.

МЕСЯЦЫ (треугольник = энергия месяца + аркан года = итог):
${lines.join('\n')}

${STYLE_RULES}
- Каждый месяц это отдельная трактовка, не копия годового прогноза и не копия соседних месяцев.

Верни СТРОГО JSON без markdown (ключи - номера месяцев):
{
${shape.join(',\n')}
}`;
}
...
async function callAI(prompt: string, apiKey: string, maxTokens: number): Promise<Record<string, unknown>> {
...
export async function generateYearReading(
  ...
  // Всё генерится параллельно и показывается вместе под одним экраном загрузки:
  // основной разбор, расширенный VIP-анализ и месяцы (двумя частями для надёжности).
  const [core, ext, m1, m2] = await Promise.all([
    callAI(buildCorePrompt(forecast, name), apiKey, 12000),
    callAI(buildExtendedPrompt(forecast, name), apiKey, 8000),
    callAI(buildMonthsPrompt(forecast, name, 1, 6), apiKey, 9000),
    callAI(buildMonthsPrompt(forecast, name, 7, 12), apiKey, 9000),
  ]);
  const months = { ...(m1 as Record<string, unknown>), ...(m2 as Record<string, unknown>) };
  const reading = { ...core, ...ext, months } as unknown as AIYearReading;
